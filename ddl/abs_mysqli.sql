...
-- PROCEDIMIENTO PARA AGREGAR HORARIOS
DROP PROCEDURE IF EXISTS AgregarHorarios;
DELIMITER //
CREATE PROCEDURE AgregarHorarios(
    IN TIPO_DE_HORARIO VARCHAR(25),
    IN HORA_MINIMA TIME,
    IN HORA_MAXIMA TIME,
...
BEGIN
	...
	(TIPO_DE_HORARIO, HORA_MINIMA, HORA_MAXIMA, OBSERVACIONES);
END //
DELIMITER ;
...
CREATE TABLE Credenciales(
    REGIDUUID VARCHAR(60) PRIMARY KEY,
...
-- TRIGGER PARA COLOCAR UUID AUTOMATICAMENTE EN TABLA CREDENCIALES
DROP TRIGGER IF EXISTS before_insert_Credenciales;
DELIMITER //
CREATE TRIGGER before_insert_Credenciales
BEFORE INSERT ON Credenciales
FOR EACH ROW
BEGIN
    IF NEW.REGIDUUID IS NULL THEN
        SET NEW.REGIDUUID = UUID();
    END IF;
END //
DELIMITER ;
...
-- TRIGGER PARA COLOCAR UUID AUTOMATICAMENTE EN TABLA Prestamos
DROP TRIGGER IF EXISTS before_insert_Prestamos;
DELIMITER //
CREATE TRIGGER before_insert_Prestamos
...
-- TRIGGER PARA AGREGAR LA FECHA DE CALCULO
DROP TRIGGER IF EXISTS before_insert_Registros;
DELIMITER //
CREATE TRIGGER before_insert_Registros
BEFORE INSERT ON Registros
FOR EACH ROW
BEGIN
    IF NEW.FECHA_PROMESA_DE_ENTREGA IS NULL THEN
        SET NEW.FECHA_PROMESA_DE_ENTREGA = DATE_ADD(CURRENT_DATE, INTERVAL 3 DAY);
    END IF;
END //
